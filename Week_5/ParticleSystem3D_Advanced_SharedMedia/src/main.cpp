#line 1 "ParticleSystem3D_Advanced_SharedMedia"
...
#line 4 "ParticleSystem3D_Advanced_SharedMedia"
...
int numParticlesToLaunch = 0; // We will set this once and then we will decrement it each time a particle is launched.
int nextParticleLaunchTime = 0; // The next time we will launch a particle (this is reset each time we launch a new particle.
int particleLaunchInterval = 100; // The number of milliseconds between our particle launch.
...
void update() {
    ...
    if (numParticlesToLaunch > 0)
    {
        // Check to see if there are any that need to be launched in our stream.
        int now = ofGetElapsedTimeMillis();
    
        // If now is greater than the next particle launch time then ...
        if (now > nextParticleLaunchTime)
        {
            // Launch a particle ...
            createAssimpParticle();

            // Take away one particle ...
            numParticlesToLaunch--;

            // Set the countdown for the next particle.
            nextParticleLaunchTime = now + particleLaunchInterval;
        }        
    }    
}
...
void keyPressed(int key)
{
    ...
    if ('s' == key)
    {
        // Stream 10 more particles.
        numParticlesToLaunch += 10; // We add 10 to our queue.
    }
}
